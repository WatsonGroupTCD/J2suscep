Module modul_lib 
 ...
 Subroutine solv()
...
#ifdef formal
        Integer(kind = int_kind), Dimension(:), Allocatable :: poss
        !poss keeps track of equation sets that give absurd values even after
        !all the checks that have been implemented
        Real(kind = real_kind), Dimension(:), Allocatable :: enrg_tb_slvd2
        Real(kind = real_kind), Dimension(:,:), Allocatable :: jval_tb_solvd2
        ...
        Do i = 1, num_spin_dens_set ! This loop changes the reference equation
                ...
                loc_energy = energy
                loc_jval = jval
                loc_energy = (loc_energy - energy(i)) * 2.19474625 * 10**5 !hartree to cm-1 
   ...
   !!!!! Printing Ends !!!!!
                        ...
                        If ((singular .eqv. .TRUE.) .OR. (identical .eqv. .TRUE.)) then
                                enrg_tb_slvd = 0
                        Else
                                Do While((singular .eqv. .FALSE.) .AND. (identical .eqv. .FALSE.) .AND. (normal .eqv. .FALSE.))
 ...
 !The checkpos subroutine ensures that ctr cycles through only
...
#ifdef formal
 !The following subroutine solves the set of equations generated by the
 !solv subroutine using a LAPACK routine
 ...
 !The following subroutine calculates the average of all the valid set of
 !solutions
 Subroutine avrg_calcs()
        Integer(kind = int_kind) :: k, l, m, x, counter
        Real(kind = real_kind), Dimension(:), Allocatable :: avg

#ifdef formal
        Integer(kind = int_kind) :: zero, sin_chk
        zero = 0
#else
        Integer(kind = int_kind) :: poss_comb
        x = num_spin_dens_set - 1
        poss_comb = comb(x,(Size(jpos) - 1))
#endif
 ...
 !!!Implementation of standard deviation!!!
         ...
         Subroutine std_dev(avg)
                ...
                Real(kind = real_kind), Dimension(:), Allocatable :: avg, var 
         ...
         !The following will check if all the valid J value sets are within 3 standard
         ...
         !The following will calculate the new average for J values since some have been
