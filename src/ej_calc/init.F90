Module init
 ...
 Integer(kind = int_kind) :: ios, num_spin_dens_set, try, poss_comb, main_cntr
 ...
 !try helps in keeping the program running just in case there is an
 !equation set is encountered that cannot be solved (ej_calc_form).
 ...
 Character (Len = 100) :: FileName1, FileName2, inp_file, read_line, Outfile
 Logical :: EndOfFile, stdv

#ifdef spin_density
   Integer(kind = int_kind), Dimension(:,:), Allocatable :: singul
   !singul stores the location of singular dets of equations identified by
   !the formal spin program. It essentially stores the data in the file singul
   Logical :: exis
#else
   Logical :: singular, normal, identical
#endif
...
!!!Initialization Subroutines!!!
         Subroutine Init1()
           ...
           !density file   !!!!!
                ...
                Do While (EndofFile .neqv. .True.)
                        If (ios .eq. Iostat_End) then
                                EndOfFile = .True.
                                Exit
                        End If
                        If ((read_line(1:5) .ne. '     ').AND.(EndofFile .neqv. .True.)) Then
                                i = i + 1
           ...
           !!!!! Determining the number of J values, reading them and printing them!!!!!
                ...
                !the total number of interaction that will help in comparisons later
           ...
           !-2S1S2
                ...
                !been
                !multiplied by 0.5 in the end to take that part into account.
                                End If
                        End Do
                End Do
